...
const sessionKey = "medical.portal.session";
...
function buildSessionFromBackend({ user, room, token }) {
  ...
  return {
    user: {
      ...
      role: user.isAdmin ? "Admin" : "Patient",
      sex: resolvedSex,
...
function storeSession(session) {
  localStorage.setItem(sessionKey, JSON.stringify({ session, view: "dashboard" }));
}
...
export async function createFileShareLink({ fileId, token }) {
  ...
  const response = await fetch("/api/patients/file-share-link", {
    method: "POST",
    headers,
