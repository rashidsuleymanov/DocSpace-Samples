...
async function uploadFileToFolder({ baseUrl, token, uploadTemplate, folderId, fileName, buffer, contentType }) {
  const pathName = String(uploadTemplate || "").replace("{folderId}", encodeURIComponent(String(folderId)));
  if (!pathName.startsWith("/")) throw new Error("uploadTemplate must start with '/'");

  const form = new FormData();
  form.append("file", new Blob([buffer], { type: contentType || "application/octet-stream" }), fileName);

  return docspaceRequest(baseUrl, token, pathName, { method: "POST", body: form });
}
...
app.post("/api/docspace/self", async (req, res) => {
  try {
    ...
    console.error("[api] /api/docspace/self failed", error);
...
app.post("/api/db/tables", async (req, res) => {
  ...
  try {
    ...
    console.error("[api] /api/db/tables failed", error);
...
app.post("/api/db/rows", async (req, res) => {
  ...
  try {
    ...
    console.error("[api] /api/db/rows failed", error);
    res.status(500).json({ ok: false, error: error?.message });
  } finally {
    try { await client?.end(); } catch {}
  }
...
app.post("/api/docspace/create-xlsx", async (req, res) => {
  ...
  try {
    ...
    console.error("[api] /api/docspace/create-xlsx failed", error);
    res.status(500).json({ ok: false, error: error?.message, details: error?.details, status: error?.status });
  }
});

app.post("/api/docspace/upload", async (req, res) => {
  const { docspace, file } = req.body || {};
  const baseUrl = docspace?.baseUrl;
  const token = docspace?.token;
  const roomId = docspace?.roomId;
  const folderTitle = docspace?.folderTitle || "Reports";
  const uploadTemplate = docspace?.uploadTemplate || "/api/2.0/files/{folderId}/upload";
  ...
  try {
    ...
    const uploaded = await uploadFileToFolder({ baseUrl, token, uploadTemplate, folderId: folder.id, fileName, buffer, contentType });
    res.json({ ok: true, folder, uploaded });
  } catch (error) {
    console.error("[api] /api/docspace/upload failed", error);
    res.status(500).json({ ok: false, error: error?.message, details: error?.details, status: error?.status });
  }
});
