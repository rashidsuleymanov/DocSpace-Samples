...
router.get("/session", async (_req, res) => {
  try {
    ...
    if (!officer) {
      return res.status(404).json({ error: "Manager is not configured" });
    }
...
router.get("/applications", (req, res) => {
  const roomId = req.demoSession?.client?.room?.id || null;
  const items = listApplications({ roomId, status: "Submitted" });
...
router.get("/applications/:id", async (req, res) => {
  try {
    ...
    const roomId = req.demoSession?.client?.room?.id || null;
    if (roomId && String(record.roomId) !== String(roomId)) {
      return res.status(404).json({ error: "Application not found" });
    }
...
router.post("/applications/:id/issue-docx", async (req, res) => {
  try {
    ...
    const file = await createFolderDocument({
      ...
      title: `Manager Summary ${date}.docx`
    });
    const issued = file
      ? {
          id: file.id,
