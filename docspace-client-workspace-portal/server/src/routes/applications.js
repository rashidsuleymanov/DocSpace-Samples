...
router.post("/", async (req, res) => {
  try {
    ...
    const rootFolder = await ensureRoomFolderByTitle(roomId, "Projects");
    if (!rootFolder?.id) {
      return res.status(500).json({ error: "Projects folder not found" });
    }
    ...
    const record = createApplicationRecord({
      roomId,
      roomTitle: room?.title || "Client Workspace",
      user: {
