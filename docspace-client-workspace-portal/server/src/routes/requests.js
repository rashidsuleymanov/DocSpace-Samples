...
router.post("/", async (req, res) => {
  try {
    ...
    const rootFolder = await ensureRoomFolderByTitle(roomId, "Action Items");
    if (!rootFolder?.id) {
      return res.status(500).json({ error: "Action Items folder not found" });
    }
    const shortId = Math.random().toString(36).slice(2, 7).toUpperCase();
    const date = new Date().toISOString().slice(0, 10);
    const folderTitle = `Action Item - ${title} - ${date} - ${shortId}`;
