...
app.use((req, _res, next) => {
  ...
  const session = sessionId ? getDemoSessionById(sessionId) : null;
...
if (configErrors.length) {
  console.warn("[client-workspace-portal] config warnings:");
...
app.use((err, _req, res, _next) => {
  console.error("[client-workspace-portal] unhandled error", err);
...
async function start() {
  ...
  app.listen(port, () => {
    console.log(`[client-workspace-portal] ${isProd ? "prod" : "dev"} on http://localhost:${port}`);
  });
}
