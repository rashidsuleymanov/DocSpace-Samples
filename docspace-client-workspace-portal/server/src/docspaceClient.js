...
const managerEmail = config.officerEmail;
const managerAccess = config.officerAccess;
const clientAccess = config.patientAccess;
...
export async function createCitizenRoom({ fullName }) {
  return apiRequest("/api/2.0/files/rooms", {
    method: "POST",
    body: {
      title: `${fullName} - Client Workspace`,
      roomType: 2
    }
  });
}
...
export async function getOfficerProfile() {
  ...
  return getUserByEmail(managerEmail);
}
...
export async function ensureRoomMembers({ roomId, citizenId }) {
  ...
  if (citizenId) {
    invitations.push({ id: citizenId, access: clientAccess });
  }

  if (managerEmail) {
    const officer = await getUserByEmail(managerEmail);
    if (officer?.id) {
      invitations.push({ id: officer.id, access: managerAccess });
    } else {
      invitations.push({ email: managerEmail, access: managerAccess });
    }
  }
...
export async function setFileExternalLink(fileId, auth) {
  ...
  try {
    ...
    // Client user tokens often cannot create external links; retry with admin token.
...
export async function getRoomInfo(roomId, auth) {
  ...
  return {
    ...
    title: content?.title || "Client Workspace",
